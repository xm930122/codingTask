...
import { TokenInfo, ColumnConfig, TrendingTokensTableProps, sortConfig } from '../types/token';
...
export const TrendingTokensTable: React.FC<TrendingTokensTableProps> = ({ tokens }) => {
  ...
  const getSortIcon = (columnKey: string) => {
    ...
    return sortConfig.direction === 'asc' ? <SortIcon className="w-3 h-3 text-up-blue-600" /> : <SortIcon className="w-3 h-3 text-down-blue-600" />;
  };
  ...
  const renderComButton = (column: ColumnConfig, key: string, idx: number) => {
    ...
    const lable = isMultiSort ? (idx !== 0 ? `/${labelArr[idx]}` : labelArr[0]) : column.label;
    return (
       <button
          key={key}
        ...
        >
          ...
          {column.sortable && (
            <span className="ml-1 flex-none rounded ">
              {getSortIcon(key)}
            </span>
          )}
       </button>
      ...
      case 'token':
        return (
          <div className="text-primary flex items-center space-x-3" >
            <div className="flex-shrink-0 h-10 w-10 flex items-center justify-center text-white font-bold text-sm">{token.baseSymbol.slice(0, 3)}</div>
            <div>
              <div className="text-sm font-medium text-primary flex items-center space-x-2">
                <span className="truncate span-class text-xs">{token.baseSymbol}</span>
                <SearchIcon className="w-3 h-3 search-icon" onClick={() => searchFunction(token)} />
              </div>
              <div className="text-sm text-secondary flex items-center space-x-2">
                <span className="truncate span-class text-xs">{token.baseToken}</span>
                <CopyIcon className="w-4 h-4 copy-icon" onClick={() => copyFunction(token.baseToken)}/></div>
            </div>
          </div>
        );
      case 'price':
        return (
          <span className="inline-flex items-center px-2.5 py-0.5 text-xs font-medium" >
            {`$${token.price < 0.01 ? formatNumber(token.price, 5) : formatNumber(token.price, 4)}`}
          </span>
        );
      case 'percent':
        return (
          <span className={`inline-flex items-center px-2.5 py-0.5 text-xs font-medium ${getPriceChangeColor(changeValue)}`} >
        ...
        return (
            <Fragment>
              ...
              <div className="text-sm text-secondary flex justify-center items-center">
                <span className="truncate span-class text-xs text-upside">{token.buyCount24h}</span>
                <span className="span-class">/</span>
                <span className="truncate span-class text-xs text-downside">{token.sellCount24h}</span>
              </div>
            </Fragment>
        );
      case 'special':
            ...
            <tr>
              {visibleColumns.map((column) => (
                <th
                  ...
                  } ${column.width ? `w-${column.width}` : ''} ${column.key.includes('$$$') ? 'th-special' : ''}`}
                >
                  {renderHeader(column)}
                </th>
              ))}
                ...
                {visibleColumns.map((column) => (
                  <td
                    ...
                    className={`px-2 py-2 text-sm ${
                      ...
                      column.key.startsWith('priceChange') ? 'whitespace-nowrap' : ''
                    }`}
                  >
                    <div 
                    className={`border-r border-r-border tdClass ${column.align === 'right' ? (rowArr.includes(column.type) ? 'flex justify-end column-class' : 'flex justify-end'): ''}`}
                    >
